  ...
  return {
    ...
    transform(code,id){
      ...
      if(out.includes(importAnchor)&&out.includes('abrirExcluirConciliacao')){
        const novo=importAnchor+`{imp&&<Card style={{marginBottom:12,background:'#FFF7F7',border:'1px solid #FECACA'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:10}}><div><b style={{fontSize:13,color:'#991B1B'}}>Conciliação pendente</b><div style={{fontSize:11,color:C.ink3,marginTop:3}}>{imp.arquivo||'Arquivo importado'} • você pode excluir e começar novamente</div></div><button onClick={abrirExcluirConciliacao} style={{border:'1px solid #FCA5A5',background:'#fff',color:'#B91C1C',borderRadius:10,padding:'8px 10px',fontSize:11,fontWeight:800,whiteSpace:'nowrap'}}>Excluir conciliação</button></div></Card>}`;
