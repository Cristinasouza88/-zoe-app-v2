...
const collect=(node,out,seen,courseUrl,moduleName='Comece por aqui')=>{if(!node||typeof node!=='object')return;const title=first(node,['title','name','module_name','section_name']),hasLessons=['lessons','contents','items'].some(key=>Array.isArray(node[key])),nextModule=title&&hasLessons&&!isLesson(node)?title:moduleName;if(isLesson(node)){const id=node.id??node.lesson_id??node.lessonId,lessonTitle=first(node,['title','name','lesson_name','lesson_title']),key=`${id||''}:${lessonTitle}`;if(!seen.has(key)){seen.add(key);const link=first(node,['url','external_url','link','permalink']);out.push({titulo:lessonTitle,modulo:moduleName,minutos:minutes(node),url:/^https?:\/\//i.test(link)?link:courseUrl})}}for(const value of Object.values(node))collect(value,out,seen,courseUrl,nextModule)};
