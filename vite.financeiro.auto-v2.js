  ...
  return {
    ...
    transform(code,id){
      ...
      if(out.includes(prepareMarker)&&!out.includes('const zoeAutoOk=')){
        const helpers=` const zoeAutoOk=t=>!t?.duplicado&&!t?.possivelDuplicado&&!t?.possivelPagamentoFatura&&!!t?.categoria&&t.categoria!=='Outros';
 const zoeNormalizarAuto=t=>{const z=regras(t);return zoeAutoOk(z)?{...z,revisar:false,confianca:z.confianca==='CONFIRMADO'||z.confianca==='CONFIRMADO_MANUAL'?z.confianca:'CONFIRMADO_AUTO'}:z};
 const zoeRegistrarAutomaticos=(itens,arquivo)=>{if(!itens?.length)return;atualiza(f=>{const vistos=new Set((f.transacoes||[]).map(chave));const novas=(itens||[]).map(zoeNormalizarAuto).filter(t=>{const k=chave(t);if(vistos.has(k))return false;vistos.add(k);return true});if(!novas.length)return f;return{...f,transacoes:[...(f.transacoes||[]),...novas],documentos:[...(f.documentos||[]),{id:uid('doc-auto'),nome:arquivo,mes:'auto',itens:novas.length,data:hoje(),automatico:true}],importacoesConciliadas:[...(f.importacoesConciliadas||[]),{arquivo,mes:'auto',itens:novas.length,data:hoje(),automatico:true}]}})};
