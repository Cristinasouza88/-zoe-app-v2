...
export default function Cursos({ d, up, aviso, voltar }) {
  ...
  const [emailCurso, setEmailCurso] = useState('');
  const [senhaCurso, setSenhaCurso] = useState('');
  const [conectando, setConectando] = useState(false);
  const [refletindo, setRefletindo] = useState(null);
  ...
  const criar = () => {
    ...
    setCursoId(novo.id); setTela('curso'); aviso('Trilha criada pela NIIL');
  };
  ...
  const conectarCurso = async () => {
    if (!emailCurso.trim() || !senhaCurso) return setErro('Informe o e-mail e a senha usados na plataforma do curso.');
    setErro(''); setConectando(true);
    try {
      const resp = await fetch('/.netlify/functions/import-protected-course', {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ url: resultado.url, email: emailCurso.trim(), password: senhaCurso })
      });
      const dados = await resp.json();
      if (!resp.ok) throw new Error(dados.erro || 'Não foi possível acessar o curso.');
      setResultado(dados); setNome(dados.nome || nome); setEmailCurso('');
    } catch (e) { setErro(e.message || 'Não foi possível acessar o curso.'); }
    finally { setSenhaCurso(''); setConectando(false); }
  };
  ...
  if (tela === 'adicionar') return (
    <div style={{ padding: '18px 16px 120px', minHeight: '100vh', background: '#F7FAF9' }}>
      ...
      <p style={{ color: C.ink3, fontSize: 12.5, lineHeight: 1.55, margin: '6px 0 18px' }}>Cole o link de um curso ou de uma playlist. A NIIL procura as aulas e organiza a jornada.</p>
      <Card style={{ padding: 18 }}>
        ...
        <Btn onClick={analisar} disabled={analisando} style={{ width: '100%', padding: 15 }}>{analisando ? <LoaderCircle className="niil-gira" size={18} style={{ verticalAlign:'middle',marginRight:7 }}/> : <Search size={17} style={{ verticalAlign:'middle',marginRight:7 }}/>} {analisando ? 'Buscando aulas…' : 'Buscar conteúdo'}</Btn>
        ...
        {erro && <div className="niil-surge" style={{ marginTop:13,padding:12,borderRadius:13,background:'#FFF1F1',color:'#A43C3C',fontSize:11.5,lineHeight:1.45 }}>{erro}</div>}
      </Card>
      {(resultado?.tipo === 'youtube' || resultado?.tipo === 'entregadigital') && <Card cls="niil-surge" style={{ marginTop:14,border:'1px solid #E7E4EA' }}><div style={{ display:'flex',gap:11,alignItems:'center',marginBottom:13 }}><div style={{ width:42,height:42,borderRadius:14,background:'#FFEEEE',color:'#E62B24',display:'grid',placeItems:'center' }}><PlaySquare size={22}/></div><div style={{ flex:1 }}><div style={{ color:C.ink,fontSize:14,fontWeight:900 }}>{resultado.nome}</div><div style={{ color:C.ink3,fontSize:10.5,marginTop:3 }}>{resultado.aulas.length} vídeos encontrados · {resultado.aulas.reduce((n,a)=>n+a.minutos,0)} min</div></div></div><Campo label="Nome da trilha" value={nome} onChange={e=>setNome(e.target.value)}/><Campo label="Minutos disponíveis por dia" type="number" min="2" value={minutos} onChange={e=>setMinutos(e.target.value)}/><div style={{ maxHeight:180,overflow:'auto',borderTop:`1px solid ${C.line}`,margin:'2px 0 14px',paddingTop:8 }}>{resultado.aulas.slice(0,12).map((a,i)=><div key={a.url} style={{ display:'flex',gap:8,padding:'7px 2px',fontSize:10.5,color:C.ink2 }}><strong style={{ color:C.roxo }}>{i+1}</strong><span style={{ flex:1 }}>{a.titulo}</span><span style={{ color:C.ink3 }}>{a.minutos} min</span></div>)}{resultado.aulas.length>12&&<div style={{ textAlign:'center',fontSize:10,color:C.ink3,padding:7 }}>+ {resultado.aulas.length-12} aulas</div>}</div><div style={{ margin:'12px 0',padding:14,borderRadius:16,background:'#F7F8F5' }}><div style={{ color:C.ink,fontSize:13,fontWeight:900 }}>Seu acordo com este curso</div><div style={{ color:C.ink3,fontSize:10.5,lineHeight:1.4,margin:'4px 0 10px' }}>A NIIL usa isso quando aparecer vontade de adiar.</div><div style={{ color:C.ink2,fontSize:10.5,fontWeight:850,marginBottom:6 }}>O que você quer ganhar?</div><div style={{ display:'flex',flexWrap:'wrap',gap:6,marginBottom:11 }}>{['Mais energia','Evoluir na carreira','Melhorar meus hábitos','Cuidar da mente'].map(x=><button key={x} onClick={()=>setGanhoCurso(x)} style={{ border:ganhoCurso===x?'2px solid #17151D':'1px solid #E7E4EA',background:ganhoCurso===x?'#F3F9DB':'#fff',color:C.ink,borderRadius:20,padding:'7px 9px',fontFamily:'inherit',fontSize:9.5,fontWeight:800 }}>{x}</button>)}</div><div style={{ color:C.ink2,fontSize:10.5,fontWeight:850,marginBottom:6 }}>O que costuma fazer você adiar?</div><div style={{ display:'flex',flexWrap:'wrap',gap:6,marginBottom:11 }}>{['Falta de tempo','Cansaço','Aula longa','Esqueço de começar'].map(x=><button key={x} onClick={()=>setObstaculoCurso(x)} style={{ border:obstaculoCurso===x?'2px solid #6C9700':'1px solid #D4E2E0',background:obstaculoCurso===x?'#DFF7F3':'#fff',color:C.ink,borderRadius:20,padding:'7px 9px',fontFamily:'inherit',fontSize:9.5,fontWeight:800 }}>{x}</button>)}</div><div style={{ color:C.ink2,fontSize:10.5,fontWeight:850,marginBottom:6 }}>Quando eu pensar em adiar, farei pelo menos:</div><div style={{ display:'flex',gap:6 }}>{['2 minutos','5 minutos','10 minutos'].map(x=><button key={x} onClick={()=>setAcordoCurso(x)} style={{ flex:1,border:acordoCurso===x?'2px solid #A7F20B':'1px solid #DDE5DD',background:acordoCurso===x?'#F3F9DB':'#fff',color:C.ink,borderRadius:12,padding:'8px 4px',fontFamily:'inherit',fontSize:9.5,fontWeight:850 }}>{x}</button>)}</div><div style={{margin:'12px 0',padding:14,borderRadius:16,background:'#ECF8F6'}}><div style={{color:C.ink,fontSize:13,fontWeight:900}}>Quando isso entra na sua agenda?</div><div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4,margin:'10px 0'}}>{DIAS_CURSO.map(dia=><button key={dia.n} onClick={()=>setDiasCurso(ds=>ds.includes(dia.n)?ds.filter(x=>x!==dia.n):[...ds,dia.n])} style={{height:35,border:0,borderRadius:9,background:diasCurso.includes(dia.n)?C.petroleo:'#fff',color:diasCurso.includes(dia.n)?'#fff':C.ink3,fontFamily:'inherit',fontSize:8.5,fontWeight:850}}>{dia.n.slice(0,1)}</button>)}</div><Campo label="Horário do lembrete" type="time" value={horaCurso} onChange={e=>setHoraCurso(e.target.value)}/></div></div><Btn onClick={importarResultado} style={{ width:'100%',padding:15 }}><Sparkles size={17} style={{verticalAlign:'middle',marginRight:7}}/>Assumir acordo e criar trilha</Btn></Card>}
      {resultado?.tipo === 'protegido' && <Card cls="niil-surge" style={{ marginTop:14,border:'1px solid #D9EFEB' }}><div style={{ display:'flex',gap:11,alignItems:'center' }}><div style={{ width:44,height:44,borderRadius:15,background:C.aquaSuave,color:C.petroleo,display:'grid',placeItems:'center' }}><ShieldCheck size={22}/></div><div><div style={{ color:C.ink,fontSize:14,fontWeight:900 }}>Plataforma protegida</div><div style={{ color:C.ink3,fontSize:10.5,marginTop:3 }}>{resultado.plataforma}</div></div></div>{resultado.plataforma.endsWith('entregadigital.app.br') ? <><p style={{ color:C.ink2,fontSize:12,lineHeight:1.5,margin:'13px 0' }}>Entre com os mesmos dados que você usa no curso. A NIIL vai ler as aulas e montar sua trilha.</p><Campo label="E-mail do curso" type="email" autoComplete="username" value={emailCurso} onChange={e=>setEmailCurso(e.target.value)}/><Campo label="Senha do curso" type="password" autoComplete="current-password" value={senhaCurso} onChange={e=>setSenhaCurso(e.target.value)}/><Btn onClick={conectarCurso} disabled={conectando} style={{ width:'100%',padding:14 }}>{conectando ? <LoaderCircle className="niil-gira" size={17} style={{verticalAlign:'middle',marginRight:7}}/> : <LogIn size={17} style={{verticalAlign:'middle',marginRight:7}}/>}{conectando?'Entrando e buscando aulas…':'Entrar e importar curso'}</Btn><div style={{ color:C.ink3,fontSize:10,lineHeight:1.45,marginTop:10 }}>Sua senha é usada somente nesta conexão e não é salva pela NIIL.</div></> : <><p style={{ color:C.ink2,fontSize:12,lineHeight:1.5,margin:'13px 0' }}>Esta plataforma ainda não possui um conector compatível.</p><Btn disabled style={{ width:'100%',padding:14 }}>Conector em preparação</Btn></>}</Card>}
  ...
  if (tela === 'curso' && curso) {
    ...
    return (
      <div style={{ paddingBottom: 120, minHeight: '100vh', background: '#F7F8F5' }}>
        <div style={{ padding: '18px 16px 12px' }}>
          ...
          <Card style={{ background: '#17151D', color: '#fff', padding: 18, boxShadow: '0 12px 26px rgba(23,21,29,.10)' }}>
        ...
        <div style={{ position: 'relative', height: alturaTrilha, margin: '48px 14px 10px' }}>
          <svg viewBox={`0 0 100 ${alturaTrilha}`} preserveAspectRatio="none" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }} aria-hidden="true">
            ...
            {curso.aulas.slice(0,-1).map((a,i) => a.feito && <line key={a.id} x1={pos[i%pos.length]} y1={ys[i]+42} x2={pos[(i+1)%pos.length]} y2={ys[i+1]+42} stroke="#17151D" strokeWidth="3" vectorEffect="non-scaling-stroke"/>)}
          </svg>
          {curso.aulas.map((a, i) => {
            ...
            return <div key={a.id} style={{ position: 'absolute', top: ys[i], left: `${pos[i%pos.length]}%`, transform: 'translateX(-50%)', width: 160, textAlign: 'center', opacity: liberada ? 1 : .48 }}>
              {inicioModulo&&<div style={{position:'absolute',top:-66,left:'50%',transform:'translateX(-50%)',width:280,padding:'10px 14px',borderRadius:16,background:liberada?'#17151D':'#D9DEDD',color:'#fff',boxShadow:'0 8px 20px rgba(23,21,29,.14)'}}><div style={{fontSize:8.5,fontWeight:900,letterSpacing:1}}>FASE {modulos.indexOf(modulo)+1}</div><div style={{fontSize:13,fontWeight:900,marginTop:2}}>{modulo}</div>{curso.recompensas?.[modulo]&&<div style={{fontSize:9.5,marginTop:3,opacity:.9}}>Recompensa: {curso.recompensas[modulo]}</div>}</div>}
              ...
              <button disabled={!liberada} onClick={() => liberada && window.open(a.url || curso.url, '_blank', 'noopener,noreferrer')} style={{ width: agora ? 82 : 74, height: agora ? 82 : 74, borderRadius: '50%', border: agora ? '8px solid #fff' : '6px solid #fff', background: a.feito ? '#17151D' : '#E1E6E5', color: a.feito ? '#fff' : '#A8B1B1', boxShadow: agora ? '0 0 0 5px rgba(142,45,226,.28),0 9px 0 rgba(120,132,132,.25)' : '0 7px 0 rgba(46,61,67,.16)', display: 'grid', placeItems: 'center', margin: 'auto', cursor: liberada ? 'pointer' : 'default' }}>
  ...
  return (
    <div style={{ padding: '20px 16px 120px', minHeight: '100vh', background: '#F7FAF9' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 18 }}><div><h1 style={{ margin: 0, color: C.ink, fontSize: 24 }}>Minhas trilhas</h1><p style={{ color: C.ink3, fontSize: 12.5, margin: '5px 0 0' }}>Seus cursos organizados pela NIIL</p></div><button onClick={() => setTela('adicionar')} style={{ width: 44, height: 44, border: 0, borderRadius: 15, background: C.petroleo, color: '#fff', display: 'grid', placeItems: 'center' }}><Plus size={21}/></button></div>
