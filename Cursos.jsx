...
export default function Cursos({ d, up, aviso, voltar }) {
  ...
  if (tela === 'adicionar') return (
    <div style={{ padding: '18px 16px 120px', minHeight: '100vh', background: '#F7FAF9' }}>
      ...
      {resultado?.tipo === 'protegido' && <Card cls="zoe-surge" style={{ marginTop:14,border:'1px solid #D9EFEB' }}><div style={{ display:'flex',gap:11,alignItems:'center' }}><div style={{ width:44,height:44,borderRadius:15,background:C.aquaSuave,color:C.petroleo,display:'grid',placeItems:'center' }}><ShieldCheck size={22}/></div><div><div style={{ color:C.ink,fontSize:14,fontWeight:900 }}>Plataforma protegida</div><div style={{ color:C.ink3,fontSize:10.5,marginTop:3 }}>{resultado.plataforma}</div></div></div><p style={{ color:C.ink2,fontSize:12,lineHeight:1.5,margin:'13px 0' }}>A ZOE identificou que este curso exige login. O conector dessa plataforma ainda precisa ser ativado para retornar à ZOE e importar as aulas com segurança.</p><Btn disabled style={{ width:'100%',padding:14 }}>Conector {resultado.plataforma.includes('hotmart')?'Hotmart':'da plataforma'} em preparação</Btn><a href={resultado.url} target="_blank" rel="noopener noreferrer" style={{ display:'block',textAlign:'center',color:C.petroleo,fontSize:11,fontWeight:800,marginTop:12 }}>Abrir apenas para assistir <ExternalLink size={13} style={{verticalAlign:'middle',marginLeft:4}}/></a><div style={{ color:C.ink3,fontSize:10.5,lineHeight:1.45,marginTop:10 }}>Sua senha não será solicitada enquanto a conexão automática não estiver operacional.</div></Card>}
