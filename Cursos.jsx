  ...
  if (tela === 'curso' && curso) {
    ...
    return (
      <div style={{ paddingBottom: 120, minHeight: '100vh', background: 'linear-gradient(180deg,#F4EDFF 0,#F8FAF9 280px)' }}>
        ...
        {premiando&&<div className="niil-surge" style={{position:'fixed',inset:0,zIndex:85,background:'linear-gradient(160deg,#6E22C9,#0A6963)',display:'grid',placeItems:'center',padding:22}}><div style={{maxWidth:390,textAlign:'center',color:'#fff'}}><div style={{fontSize:54}}>🏆</div><div style={{fontSize:11,fontWeight:950,letterSpacing:1.3,marginTop:10}}>FASE CONCLUÍDA</div><h2 style={{fontSize:26,margin:'7px 0'}}>{premiando}</h2><p style={{fontSize:12.5,lineHeight:1.5,opacity:.85}}>Você cumpriu o combinado. Escolha agora uma recompensa real para marcar essa conquista.</p><Campo label="Minha recompensa" placeholder="Um café especial, um filme, um passeio…" value={recompensa} onChange={e=>setRecompensa(e.target.value)}/><Btn onClick={salvarRecompensa} style={{width:'100%',padding:15,background:C.lima,color:C.greenDark}}>Guardar recompensa e avançar</Btn></div></div>}
      </div>
    );
  }
