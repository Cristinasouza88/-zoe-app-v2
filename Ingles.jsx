...
const LinhaCheck = ({ marcado, onClick, children }) => (
  <div onClick={onClick} style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '9px 0', borderBottom: `1px dashed ${C.line}`, cursor: 'pointer' }}>
    {marcado ? <Check size={18} color={C.green} /> : <Circle size={18} color={C.ink3} />}
  ...
  return (
    <div style={{ padding: '20px 16px 100px', maxWidth: 520, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
        <Languages size={22} color={C.green} />
      ...
      <Card cls="zoe-surge" style={{ marginBottom: 16 }}>
        ...
        <a href={PAINEL_URL} target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 12, fontSize: 13, fontWeight: 700, color: C.green, textDecoration: 'none' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: ritmoAtual ? 10 : 0 }}>
          {RITMOS.map(r => (
            <button key={r.id} onClick={() => setRitmo(r.id)} style={{
              ...
              background: ing.ritmo === r.id ? C.green : 'transparent', color: ing.ritmo === r.id ? '#fff' : C.ink2,
      ...
      {FASES.map((f, i) => {
        ...
        return (
          <Card key={f.id} cls="zoe-surge" delay={i * 40} style={{ marginBottom: 12, opacity: feita ? .75 : 1 }}>
            ...
            <a href={f.link} target="_blank" rel="noreferrer" style={{ fontSize: 13, fontWeight: 700, color: C.green, textDecoration: 'none' }}>Abrir {f.nome} →</a>
        ...
        {DESAFIOS.map(des => (
          <div key={des.id} style={{ padding: '10px 0', borderBottom: `1px dashed ${C.line}` }}>
            <div onClick={() => alternarDesafio(des.id)} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', cursor: 'pointer' }}>
              {ing.desafiosMarcados[des.id] ? <Check size={18} color={C.green} style={{ marginTop: 2, flexShrink: 0 }} /> : <Circle size={18} color={C.ink3} style={{ marginTop: 2, flexShrink: 0 }} />}
              <div>
