  ...
  return {
    ...
    transform(code,id){
      ...
      if(id.endsWith('/App.jsx')||id.endsWith('App.jsx')){
        ...
        if(out.includes(importAnchor)&&!out.includes("import ZoeOrbCloud from './ZoeOrbCloud.jsx';")){
          out=out.replace(importAnchor,importAnchor+"\nimport ZoeOrbCloud from './ZoeOrbCloud.jsx';");
          changed=true;
        }
        ...
        if(out.includes(essencial)){
          out=out.replace(essencial,"{ id: 'essencial', nome: 'ZOE Essencial', texto: 'Mais madura, discreta e reflexiva.', imagem: avatarZoeEssencial, fundo: 'transparent', orb: true }");
          changed=true;
        }
        ...
        if(out.includes(opcaoImg)){
          out=out.replace(opcaoImg,`{opcao.orb ? <ZoeOrbCloud size={138} active={true} /> : <img src={opcao.imagem} alt="" style={{ width:'92%',height:'94%',objectFit:'contain' }} />}`);
          changed=true;
        }
        ...
        if(out.includes(trilhaImg)){
          out=out.replace(trilhaImg,`{avatarId==='essencial'?<ZoeOrbCloud size={90} />:<img src={avatarImagem} alt="ZOE" style={{ width:88,height:96,objectFit:'contain',objectPosition:'center',filter:'drop-shadow(0 8px 12px rgba(55,28,105,.18))' }}/>} `);
          changed=true;
        }
        ...
        if(out.includes(homeImg)){
          out=out.replace(homeImg,`{avatarId==='essencial'?<div style={{margin:'4px 0 6px',zIndex:1,display:'grid',placeItems:'center',width:170,height:170,borderRadius:'50%',background:'transparent',overflow:'visible'}}><ZoeOrbCloud size={164}/></div>:<img src={avatarImagem} alt="ZOE, sua coach" style={{ width:190,height:159,objectFit:'contain',margin:'-2px 0 -4px',filter:'drop-shadow(0 12px 13px rgba(74,25,133,.13))',zIndex:1 }} />}`);
          changed=true;
        }
        ...
        if(out.includes(coachImg)){
          out=out.replace(coachImg,`{avatarId==='essencial'?<ZoeOrbCloud size={188}/>:<img src={avatarImagem} alt="ZOE celebrando" style={{height:190,maxWidth:205,objectFit:'contain'}}/>}`);
          changed=true;
        }
      ...
      if(id.endsWith('/JornadaSistemica.jsx')||id.endsWith('JornadaSistemica.jsx')){
        ...
        if(out.includes(reactImport)&&!out.includes("import ZoeOrbCloud from './ZoeOrbCloud.jsx';")){
        ...
        if(out.includes(celebra)){
          out=out.replace(celebra,`{avatarId==='essencial'?<div style={{display:'grid',placeItems:'center',marginBottom:5}}><ZoeOrbCloud size={132}/></div>:<img src={avatar} alt="ZOE celebrando" style={{width:130,height:130,objectFit:'contain',filter:'drop-shadow(0 10px 12px rgba(55,28,105,.15))'}}/>}`);
          changed=true;
        }
      }
