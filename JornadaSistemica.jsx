...
function Titulo({ icone: Icone, selo, titulo, texto, progresso }) {
  return <div className="zoe-surge" style={{ marginBottom: 18 }}>
...
function EscalaRoda({ valores, onChange, cor = C.roxo }) {
  ...
  return <div>
    <Card style={{ marginBottom: 15, background: '#FBF9FF', border: '1px solid #EEE6F8' }}>
    ...
    {RODA_SETORES.map((setor, idx) => {
      ...
      return <Card key={setor} cls="zoe-surge" delay={idx * 25} style={{ marginBottom: 9, padding: 13 }}>
...
export default function JornadaSistemica({ id, d, up, campo, setCampo, aviso, avatar }) {
  ...
  const prioridade = campo('zoe-prioridade');
  ...
  if (id === 'checkinEmocional') return <div>
    <Titulo icone={Sparkles} selo="COMEÇO" progresso="1 de 8" titulo="Como você chega hoje?" texto="Não existe resposta certa. A ZOE usa este estado para ajustar o ritmo da sua jornada."/>
    ...
    {jornada.emocaoAtual && <Card cls="zoe-surge" style={{ marginTop:16,background:C.aquaSuave,border:'1px solid #CDEFE9' }}><strong style={{ color:C.petroleo,fontSize:13 }}>Entendi.</strong><p style={{ color:C.ink2,fontSize:12.5,lineHeight:1.5,margin:'5px 0 0' }}>Vamos observar o que está por trás desse estado, sem tentar corrigir você às pressas.</p></Card>}
  </div>;
  ...
  if (id === 'escolhaPrioridade') return <div>
    ...
    {(menores.length?menores:RODA_SETORES.slice(0,4)).map((s,i)=><button key={s} onClick={()=>{som();setCampo('zoe-prioridade',s)}} style={{ width:'100%',display:'flex',alignItems:'center',gap:12,padding:14,marginBottom:9,borderRadius:17,border:`2px solid ${prioridade===s?C.roxo:C.line}`,background:prioridade===s?'#FBF7FF':'#fff',fontFamily:'inherit',textAlign:'left',cursor:'pointer' }}><span style={{ width:34,height:34,borderRadius:11,background:prioridade===s?C.roxo:'#F1E8FC',color:prioridade===s?'#fff':C.roxo,display:'grid',placeItems:'center',fontWeight:900 }}>{roda1[s]||i+1}</span><span style={{ flex:1,color:C.ink,fontSize:14,fontWeight:850 }}>{s}</span>{prioridade===s&&<Check size={19} color={C.roxo}/>}</button>)}
    <Area label="Por que esta área importa agora?" value={campo('zoe-prioridade-porque')} onChange={e=>setCampo('zoe-prioridade-porque',e.target.value)} placeholder="O que mudaria na sua vida se ela avançasse um pouco?"/>
  </div>;
  ...
  if (id === 'matrizGanhosPerdas') {
    ...
    return <div><Titulo icone={Brain} selo="COMPREENDER" progresso="4 de 8" titulo="Toda mudança tem dois lados" texto={`Vamos enxergar a decisão sobre ${prioridade||'sua prioridade'} sem respostas bonitas ou culpa.`}/><div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:9 }}>{qs.map(([k,t,p,c],i)=><Card key={k} cls="zoe-surge" delay={i*60} style={{ padding:12,borderTop:`4px solid ${c}` }}><label style={{ display:'block',minHeight:34,color:C.ink,fontSize:11.5,fontWeight:900,lineHeight:1.3 }}>{t}</label><textarea value={campo(`matriz-${k}`)} onChange={e=>setCampo(`matriz-${k}`,e.target.value)} placeholder={p} style={{ width:'100%',height:96,resize:'none',border:0,outline:0,background:'#F7F9F8',borderRadius:11,padding:9,color:C.ink,fontFamily:'inherit',fontSize:12,lineHeight:1.4 }}/></Card>)}</div></div>;
  }
  ...
  if (id === 'ativarAgenda') {
    ...
    return <div><Titulo icone={CalendarDays} selo="ORGANIZAR" progresso="7 de 8" titulo="Quando isso cabe na sua semana?" texto="A ZOE colocará o experimento na agenda. Você poderá ajustar sem perder seu progresso."/><Card style={{ marginBottom:13 }}><div style={{ color:C.ink,fontSize:14,fontWeight:900,marginBottom:11 }}>{campo('exp-acao')||'Defina a ação na etapa anterior'}</div><div style={{ display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:5 }}>{DIAS.map(dia=>{const on=plano.dias?.includes(dia);return <button key={dia} onClick={()=>{som();up(s=>{const p=s.jornada?.planoSemana||plano;const ds=p.dias||[];return {...s,jornada:{...(s.jornada||{}),planoSemana:{...p,dias:on?ds.filter(x=>x!==dia):[...ds,dia]}}}})}} style={{ height:38,border:0,borderRadius:10,background:on?C.roxo:'#F1F3F2',color:on?'#fff':C.ink3,fontSize:9.5,fontWeight:850,fontFamily:'inherit' }}>{dia}</button>})}</div><div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:9,marginTop:13 }}><Campo label="Horário" type="time" value={plano.hora||'07:30'} onChange={e=>up(s=>({...s,jornada:{...(s.jornada||{}),planoSemana:{...(s.jornada?.planoSemana||plano),hora:e.target.value}}}))}/><Campo label="Minutos" type="number" value={plano.duracao||15} onChange={e=>up(s=>({...s,jornada:{...(s.jornada||{}),planoSemana:{...(s.jornada?.planoSemana||plano),duracao:+e.target.value}}}))}/></div></Card><Btn onClick={salvar} style={{ width:'100%',padding:15 }}>ATIVAR NA MINHA AGENDA</Btn>{plano.ativo&&<Card cls="zoe-surge" style={{ marginTop:12,background:C.limaSuave,border:'1px solid #DDF3B4' }}><div style={{ display:'flex',gap:9,alignItems:'center' }}><Check size={20} color={C.petroleo}/><div><strong style={{ display:'block',color:C.petroleo,fontSize:13 }}>Experimento ativo</strong><span style={{ color:C.ink2,fontSize:11 }}>{plano.dias.join(', ')} · {plano.hora} · {plano.duracao} min</span></div></div></Card>}</div>;
  }
  ...
  if (id === 'checkpointRoda') {
    ...
    return <div><Titulo icone={Target} selo="APRENDER" progresso="8 de 8" titulo="O que mudou nesta primeira volta?" texto="Refaça a fotografia. A mudança pode ser pequena; o importante é reconhecer o que os dados mostram."/><EscalaRoda valores={roda2} cor={C.green} onChange={(s,n)=>up(st=>({...st,rodas:{...st.rodas,2:{...(st.rodas?.[2]||{}),[s]:n}},jornada:{...(st.jornada||{}),checkpointEm:new Date().toISOString()}}))}/>{preenchidos===RODA_SETORES.length&&<Card cls="zoe-celebra" style={{ marginTop:16,textAlign:'center',padding:22,background:'linear-gradient(145deg,#F1FFDB,#E6FBF8)',border:'1px solid #D9F1C4' }}><img src={avatar} alt="ZOE celebrando" style={{ width:130,height:130,objectFit:'contain',filter:'drop-shadow(0 10px 12px rgba(55,28,105,.15))' }}/><h3 style={{ color:C.petroleo,fontSize:20,margin:'5px 0 7px' }}>Primeira volta concluída!</h3><p style={{ color:C.ink2,fontSize:12.5,lineHeight:1.5,margin:0 }}>Sua média foi de {inicial.toFixed(1)} para {agora.toFixed(1)}. Mais importante que subir a nota é entender o que funcionou.</p><button onClick={()=>som('ganho')} style={{ marginTop:14,border:0,borderRadius:13,background:C.petroleo,color:'#fff',padding:'11px 16px',fontWeight:900,fontFamily:'inherit' }}>OUVIR CONQUISTA</button></Card>}</div>;
  }
